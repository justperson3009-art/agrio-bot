"""
Модуль ответов для основных команд
/start, /help, /about, /status
"""


def get_start_response() -> str:
    """Ответ на команду /start"""
    return (
        "🌱 **Добро пожаловать в Agrio-Бот!**\n\n"
        "🏪 **AGRIO.BY** — профессиональные семена для вашего урожая\n\n"
        "🌐 **Сайт:** [agrio.by](https://agrio.by)\n"
        "📦 **Ozon:** [ozon.by/seller/agrio/](https://ozon.by/seller/agrio/)\n"
        "📦 **Wildberries:** [wildberries.kg/seller/4182657](https://wildberries.kg/seller/4182657)\n"
        "🎵 **TikTok:** [@agrio.by](https://www.tiktok.com/@agrio.by)\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "🤖 **Что умеет бот:**\n\n"
        "🌱 **Выращивание 70+ культур**\n"
        "• Напишите **«как сажать [культура]»** — полное руководство\n"
        "• **«как ухаживать за [культура]»** — полив, подкормка, обрезка\n"
        "• **«помидорчик»** — поймёт даже уменьшительно-ласкательные!\n\n"
        "📅 **Календарь и регионы**\n"
        "• **«календарь»** или **«что делать в апреле»**\n"
        "• **«когда сажать в Беларуси»** — сроки для вашего региона\n"
        "• **«лунный календарь»** — благоприятные дни\n\n"
        "🌍 **Регионы:** Беларусь, Сибирь, Подмосковье, Юг\n\n"
        "🔍 **Диагностика**\n"
        "• **«желтеют томаты»** — определю причину\n"
        "• **«не растёт рассада»** — найду проблему\n"
        "• **Пришлите фото + описание** — диагноз по симптомам\n\n"
        "🌾 **Агротехника**\n"
        "• **«чем подкормить»**, **«полив»** — точные нормы\n"
        "• **«сидераты»**, **«севооборот»** — что после чего\n"
        "• **«заморозки»** — защита от холода\n"
        "• **«теплица или открытый грунт»** — сравнение\n\n"
        "🛒 **Каталог**\n"
        "• Напишите **«Каталог»** — все 93 сорта AGRIO\n"
        "• **«расскажи про [сорт]»** — характеристики гибрида\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "💡 **Попробуйте:**\n"
        "• «как сажать томаты»\n"
        "• «чем подкормить огурцы»\n"
        "• «заморозки что делать»\n"
        "• «лунный календарь»\n"
        "• «Каталог»\n\n"
        "🏪 **AGRIO.BY** — качественные семена для вашего урожая!"
    )


def get_subscribe_prompt() -> str:
    """Предложение подписки после /start"""
    return (
        "📬 **Бесплатная рассылка полезных советов!**\n\n"
        "Каждый понедельник бот присылает актуальный совет по огороду:\n"
        "• Когда и что сажать\n"
        "• Как ухаживать за рассадой\n"
        "• Защита от болезней и вредителей\n"
        "• Секрики богатого урожая\n\n"
        "💰 **Это полностью БЕСПЛАТНО!**\n\n"
        "📝 **Пример совета:**\n"
        "🌱 *Март, неделя 3: Обрезка деревьев!*\n"
        "✂️ До сокодвижения\n"
        "🌳 Удаляйте сухие и больные ветки\n"
        "🧴 Смажьте срезы садовым варом\n\n"
        "Подписаться: /subscribe\n"
        "Отписаться можно в любой момент: /unsubscribe"
    )


def get_help_response() -> str:
    """Ответ на команду /help"""
    return (
        "📚 **Команды бота:**\n\n"
        "/start - Начать общение\n"
        "/help - Показать эту справку\n"
        "/catalog - Показать ВСЕ семена AGRIO по категориям\n"
        "/about - О проекте Agrio\n"
        "/status - Проверить состояние бота\n\n"
        "💡 **Как использовать:**\n"
        "• В личных чатах: просто пишите вопрос\n"
        "• В группах: упоминайте бота @Agrio_Bot\n\n"
        "🌱 **Примеры запросов:**\n"
        "• «Когда сажать томаты на рассаду?»\n"
        "• «Какие томаты лучше для теплицы?»\n"
        "• «Посоветуй ранний перец»\n"
        "• «Как ухаживать за баклажанами?»\n"
        "• «Какие сорта у вас есть?»\n\n"
        "📋 **Команда «Каталог»:**\n"
        "Просто напишите **«Каталог»** или **«Коталог»** (даже с опечаткой!)\n"
        "Бот покажет все 93 сорта семян по категориям!\n\n"
        "🛒 **Где купить:**\n"
        "• «Где купить семена?»\n"
        "• «Есть ли у вас сайт?»\n"
        "• «Как заказать?»"
    )


def get_about_response() -> str:
    """Ответ на команду /about"""
    return (
        "🏪 **Agrio** — интернет-магазин семян в Беларуси.\n\n"
        "📍 **Адрес:** аг. Ольшаны, Столинский район, Брестская область\n"
        "📞 **Телефон:** +375 29 795 99 68\n\n"
        "🌱 **В ассортименте:**\n"
        "• Томаты (29 гибридов)\n"
        "• Перец (13 сортов)\n"
        "• Морковь, капуста, арбузы\n"
        "• Кабачки, тыквы, брокколи\n"
        "• Дыни, баклажаны и др.\n\n"
        "Я — ИИ-консультант, созданный чтобы помогать вам с вопросами о растениях и подборе семян.\n\n"
        "🌐 **Сайт:** agrio.by\n"
        "💡 Если я не могу помочь, наша служба поддержки всегда на связи!"
    )


def get_status_response(bot_info: dict) -> str:
    """Ответ на команду /status"""
    return (
        "🔍 **Статус бота**\n\n"
        f"🤖 Имя: {bot_info['first_name']}\n"
        f"📛 Username: @{bot_info['username']}\n"
        f"🆔 ID: {bot_info['id']}\n\n"
        f"✅ Бот работает\n\n"
        "📋 **Настройки приватности:**\n"
        f"• Может присоединяться к группам: {'✅' if bot_info['can_join_groups'] else '❌'}\n"
        f"• Читает все сообщения в группе: {'✅' if bot_info['can_read_all_group_messages'] else '❌'}\n\n"
        "⚠️ **Если 'Читает все сообщения' = ❌:**\n"
        "1. Напишите @BotFather\n"
        "2. /setprivacy → выберите бота → Disable\n"
        "3. Удалите бота из группы и добавьте снова"
    )


def get_mention_only_prompt(bot_username: str) -> str:
    """Подсказка когда бот не упомянут в группе"""
    return (
        f"🌱 Задайте мне вопрос о растениях!\n\n"
        f"Например: @{bot_username} когда сажать томаты?"
    )


def get_moderation_block_response() -> str:
    """Ответ при блокировке модерацией"""
    return (
        "⚠️ Я специалист только по растениям и семенам!\n\n"
        "🏪 **AGRIO.BY** — магазин семян\n"
        "🌐 Сайт: agrio.by\n"
        "📦 Ozon: ozon.by/seller/agrio/\n"
        "📦 WB: wildberries.kg/seller/4182657\n\n"
        "🌱 Задайте вопрос о выращивании растений!"
    )


def get_injection_block_response() -> str:
    """Ответ при попытке prompt injection"""
    return (
        "⚠️ Я не могу изменить свои инструкции. "
        "Задайте вопрос о растениях!"
    )
